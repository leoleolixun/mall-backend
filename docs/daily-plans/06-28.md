...
如果你暂时不想用泛型，也可以写死 `ProductPageResponse`。
...
- [ ] 数据库中存在 `categories` 表
- [ ] 数据库中存在 `products` 表
- [ ] 数据库中存在 `product_skus` 表
- [ ] 默认商户 `id = 1` 已初始化
- [ ] 有测试分类数据
- [ ] 有测试商品数据
- [ ] 每个测试商品至少有一个 SKU
- [ ] `GET /api/v1/categories` 返回分类列表
- [ ] `GET /api/v1/products` 返回分页商品列表
- [ ] `GET /api/v1/products/:id` 返回商品详情
- [ ] `GET /api/v1/products/:id/skus` 返回 SKU 列表
- [ ] 商品、分类、SKU 查询都默认带 `merchant_id = 1`
